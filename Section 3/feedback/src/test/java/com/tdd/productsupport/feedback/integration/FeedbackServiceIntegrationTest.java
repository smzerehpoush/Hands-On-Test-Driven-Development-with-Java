...
class FeedbackServiceIntegrationTest {
    ...
    void testGetFeedbackById() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/feedback/{id}", 1))
                ...
                .andExpect(header().string(HttpHeaders.ETAG, "\"1\"")).andExpect(header().string(HttpHeaders.LOCATION, "/feedback/1"))

                .andExpect(jsonPath("$.id", is("1"))).andExpect(jsonPath("$.productId", is(1))).andExpect(jsonPath("$.userId", is(1))).andExpect(jsonPath("$.status", is("POSTED")));
    }
    ...
    void testFeedbackFoundForProductId() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/feedback/10")).andExpect(status().isNotFound());
    }
    ...
    void testAllFeedbackFound() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/feedback")).andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))

                .andExpect(jsonPath("$[0].id", is("1"))).andExpect(jsonPath("$[1].id", is("2"))).andExpect(jsonPath("$[0].status", is("POSTED"))).andExpect(jsonPath("$[1].status", is("PUBLISHED")));
    }
    ...
    void testSavingNewFeedback() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/feedback").contentType(MediaType.APPLICATION_JSON_VALUE).content(new ObjectMapper().writeValueAsString(newFeedback)))

                .andExpect(status().isCreated()).andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))

                .andExpect(header().string(HttpHeaders.ETAG, "\"1\"")).andExpect(header().string(HttpHeaders.LOCATION, "/feedback/1"))

                .andExpect(jsonPath("$.id", is("1"))).andExpect(jsonPath("$.status", is("POSTED"))).andExpect(jsonPath("$.version", is(1)));
    }
}
